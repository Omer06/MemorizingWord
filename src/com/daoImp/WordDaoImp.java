...
public class WordDaoImp implements WordDao{
	...
	private static final Logger log = new Logger(WordDaoImp.class);
	...
	public boolean deleteById(int wordId) {
		try{
			return sessionFactory.getCurrentSession().createQuery("Delete from word where id=:id").setParameter("id", wordId).executeUpdate() == 1;
		}
		catch(Exception e){
			return false;
		}
